...
public class Server {
	...
	/**
	 * We do a major update every 600ms.
	 */
	private static final long MAJOR_UPDATE_TIME = 600;
	 ...
	 * 
	 * These updates are done every 600 ms.
	 * 
	...
	public void processEvents() {
		...
		//1 min before restart we start the new server
		if(startupTime + 3500 * 1000 < System.currentTimeMillis() && stage == 0) {
			stage++;
			Runtime.getRuntime().exec("runserver.bat");
for(Player p : World.getInstance().getPlayerList()) {
			p.getActionSender().sendMessage("[Restarter] Restarting in 30 seconds...");
		
}}
		//10 seconds b4 restart we save everyone
		if(startupTime + 3590 * 1000 < System.currentTimeMillis() && stage == 1) {
			stage++;
			for(Player p : World.getInstance().getPlayerList()) {
				p.getActionSender().sendMessage("[Restarter] Restarting in 10 seconds...");
				p.getActionSender().sendMessage("[Restarter] If you didn't get disconnected please logout to avoid rollbacks.");
		...
		if(startupTime + 3600 * 1000 < System.currentTimeMillis() && stage == 2) {
			stage++;
System.exit(0);
		}*/
		engine.tick();
